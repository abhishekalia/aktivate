...
- **Design Theme**: Clean professional Y2K aesthetic with chrome/metallic theme. Three-tone color scheme: sky blue (#8eb5cc), warm off-white (#f0ece4), dark ink (#1e2830) in alternating sections. Chrome metallic accents throughout (gradients, borders, dots, tags). Fonts: Space Grotesk (headings), Space Mono (body), Playfair Display (chrome "Face." headline). Chrome silver keycap buttons with 3D press effect. Crosshair cursor. Chrome CSS variables: `--chrome-1` through `--chrome-4`, `--chrome-border`. Reusable `.chrome-text`, `.chrome-text-glow`, `.chrome-text-anim` utility classes. All styling defined inline in `client/index.html`
...
- **Zoom Portal**: `#zoom-scene-spacer` (300vh tall) wraps sticky `#zoom-scene` with hero text overlay + computer image. JS drives scale/translate on scroll with eased progress. `#zoom-overlay` fades in at 65% to transition into `#main-content`. Computer image: `client/public/retro-computer.png`. Screen overlay positioned at top:11.5%, left:27%, width:42%, height:27% of the image
