...
- **Mobile Hero**: Separate `#mobile-hero` section shown only on mobile (≤960px). Static layout — iMac image (55vw), label, headline, description, CTA button. No zoom/scroll animation. Desktop hero (`#hero-spacer`, `#zoom-overlay`) hidden on mobile via `display:none`.
